...
export class AccountComponent implements OnInit {
  ...
  constructor(
    /*@Inject(LOCAL_STORAGE) private localStorage: any,*/ app: TheOrangeAllianceGlobals,
  ...
  getUser() {
    if (this.firebaseUser !== null) {
      this.cloud
        .getUserData(this.firebaseUser, 'exclude-region')
        .then((user: TOAUser) => {
          this.user = user;
  ...
  linkPhone() {
    ...
    this.firebaseUser
      .linkWithPhoneNumber(phoneNumber, this.recaptchaVerifier)
      .then((confirmationResult) => {
        // SMS sent. Prompt user to type the code from the message
        confResult = confirmationResult;
        return this.getDebugInput(
      ...
      .then((result) => {
        this.db
          .object(`Phones/${phoneNumber.substr(1)}/opted`)
          .query.once('value')
          .then((items) => {
            if (items.val() !== false) {
              return this.db
      ...
      .then((resp) => {
        return this.db
          .object(`Phones/${phoneNumber.substr(1)}/uid`)
          .set(`${this.user.uid}`);
      })
      .then((resp) => {
        this.translate
          .get('pages.account.success_link', {
            name: 'phone'
          })
  ...
  generateApiKey(): void {
    if (this.emailVerified) {
      ...
      this.cloud
        .generateApiKey(this.firebaseUser)
        .then((key) => {
  ...
  sendEmailVerification() {
    this.firebaseUser
      ...
      .then(() => {
        // Show Success
        this.translate
          .get('pages.event.subpages.admin.success_sent_verify_email')
          .subscribe((success_sent: string) => {
      ...
      .catch((error) => {
        ...
        console.log(error);
        this.translate
          .get('general.error_occurred')
          .subscribe((error_string: string) => {
  ...
  sendPasswordResetEmail() {
    this.auth
      ...
      .then(() => {
        ...
        this.translate
          .get('pages.account.reset_password_email')
          .subscribe((res: string) => {
            this.snackbar.open(res);
          });
      })
    ...
    if (this.firebaseUser.providerData.length > 1) {
      this.firebaseUser
        ...
        .then((result) => {
          ...
          // Show success in snackbar
          this.translate
            .get('pages.account.success_unlink', {
              name: this.getProviderName(provider)
            })
            .subscribe((res: string) => {
              this.snackbar.open(res);
  ...
  showSnackbar(translateKey: string, errorKey?: string, value?: number) {
    ...
    const msg = isEmail
      ? 'pages.event.subpages.admin.verify_email'
      : translateKey;
        ...
        if (reason === 'action') {
          if (isEmail) {
            this.firebaseUser
              ...
              .then(() => {
                this.showSnackbar(
                  `pages.event.subpages.admin.success_sent_verify_email`
                );
              })
              .catch((err) => {
                this.showSnackbar(
  ...
  requestPermission() {
    this.messaging
      .requestPermission(this.user.firebaseUser)
      .then(() => {
        this.notifications = Notification ? Notification.permission : null;
      })
      .catch(() => {
